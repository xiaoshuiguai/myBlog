------------------------------------------------------
-- Export file for user GKPT61@10.16.3.179/HIATMPDB --
-- Created by linan3 on 2022/7/26, 18:59:51 ----------
------------------------------------------------------
...
;
comment on column KNOW_TASK_CONFIG.config_id
  is 'id';
comment on column KNOW_TASK_CONFIG.class_name
  is '类的全限定名';
comment on column KNOW_TASK_CONFIG.cron
  is 'CRON表达式';
comment on column KNOW_TASK_CONFIG.is_active
  is '是否有效 0无效 1有效';
comment on column KNOW_TASK_CONFIG.end_time
  is '结束时间';
alter table KNOW_TASK_CONFIG
  add constraint PK_DOMAIN_CONFIG primary key (CONFIG_ID);

--KNOW_RELATION_CONFIG
create table KNOW_RELATION_CONFIG
(
  rid        VARCHAR2(32) not null,
  baselabel  VARCHAR2(32),
  class_name VARCHAR2(128)
)
;
comment on column KNOW_RELATION_CONFIG.rid
  is '32位uuid';
comment on column KNOW_RELATION_CONFIG.baselabel
  is '开始节点id';
comment on column KNOW_RELATION_CONFIG.class_name
  is '类的全限定名';
alter table KNOW_RELATION_CONFIG
  add constraint PK_KNOW_RELATION primary key (RID);



prompt
prompt Creating table ADMIN_AREA
prompt =========================
prompt
...
prompt
prompt Creating table BIS_ENUM_TYPE
prompt ============================
prompt
...
prompt
prompt Creating table BIS_MENU
prompt =======================
prompt
...
prompt
prompt Creating table FACE_AREA_INDEX_REAL
prompt ===================================
prompt
create table FACE_AREA_INDEX_REAL
(
  index_id                      VARCHAR2(500) not null,
  area_id                       VARCHAR2(32),
  collect_time                  DATE,
...
prompt
prompt Creating table FACE_CROSS_DIR_INDEX_REAL
prompt ========================================
prompt
create table FACE_CROSS_DIR_INDEX_REAL
(
  index_id        VARCHAR2(40) not null,
  ints_id         VARCHAR2(32),
  approach_dir    NUMBER,
...
prompt
prompt Creating table FACE_CROSS_INDEX_REAL
prompt ====================================
prompt
create table FACE_CROSS_INDEX_REAL
(
  index_id            VARCHAR2(32) not null,
  ints_id             VARCHAR2(32),
  collect_time        DATE,
...
prompt
prompt Creating table FACE_HBASE2ORACLE_CONFIG
prompt =======================================
prompt
...
alter table FACE_INOUTCITY_CROSSING
  ...
  maxtrans 255;
alter table FACE_INOUTCITY_CROSSING
  add constraint INOUTCITY_CROSSING_UNIQUE unique (CROSSING_CODE)
  using index 
  tablespace HIATMPTS
  pctfree 10
  initrans 2
...
prompt
prompt Creating table FACE_INOUTCITY_INDEX_REAL
prompt ========================================
prompt
create table FACE_INOUTCITY_INDEX_REAL
(
  index_id           VARCHAR2(32) not null,
  total_carcnt       NUMBER,
  in_carcnt          NUMBER,
...
comment on column FACE_JAMSECTION.jam_cnt
  is '拥堵次数';
comment on column FACE_JAMSECTION.direction
  is '干线方向';
...
prompt
prompt Creating table FACE_MAINLINE_INDEX_REAL
prompt =======================================
prompt
create table FACE_MAINLINE_INDEX_REAL
(
  index_id              VARCHAR2(100) not null,
  line_id               VARCHAR2(32),
  collect_time          DATE,
...
create index INDEX_LINEREAL_TIME on FACE_MAINLINE_INDEX_REAL (COLLECT_TIME)
  ...
  initrans 2
  maxtrans 255;
alter table FACE_MAINLINE_INDEX_REAL
  add constraint PK_LINEINDEX_REAL primary key (INDEX_ID)
  using index 
  tablespace HIATMPTS
  pctfree 10
  initrans 2
...
(
  ...
  center_location VARCHAR2(100) not null,
...
(
  ...
  keyword         NVARCHAR2(200) not null,
  prompt_question NVARCHAR2(300) not null
)
tablespace HIATMPTS
  pctfree 10
  initrans 1
...
(
  question_id          VARCHAR2(32) not null,
  keyword_group        NVARCHAR2(200) not null,
  function_name        VARCHAR2(200) not null,
  function_param       VARCHAR2(200),
  answer_id            VARCHAR2(200) not null,
...
prompt
prompt Creating table FACE_SECTION_INDEX_REAL
prompt ======================================
prompt
create table FACE_SECTION_INDEX_REAL
(
  index_id        VARCHAR2(40) not null,
  section_id      VARCHAR2(32),
  collect_time    DATE,
...
prompt
prompt Creating table FACE_WARNING_ROLES
prompt =================================
prompt
...
prompt
prompt Creating table FRM_CODE
prompt =======================
prompt
...
prompt
prompt Creating table FRM_CODETYPE
prompt ===========================
prompt
...
*/
prompt
prompt Creating table LEG_EAGLEEYE_INFO
prompt ================================
prompt
...
*/
prompt
prompt Creating view FACE_AREA_INDEX_DATA_VIEW
prompt =======================================
prompt
create or replace force view face_area_index_data_view as
select t.index_id,
       t.area_id,
       t.collect_time,
       (
...
prompt
prompt Creating view FACE_AREA_INDEX_DATA_VIEW_MUL
prompt ===========================================
prompt
create or replace force view face_area_index_data_view_mul as
select t.index_id,
       t.area_id,
       t.collect_time,
       (
...
prompt
prompt Creating function CALCULATERATIO
prompt ================================
prompt
...
prompt
prompt Creating view FACE_AREA_INDEX_VIEW
prompt ==================================
prompt
create or replace force view face_area_index_view as
select t1.index_id,
       t1.area_id,
       t1.collect_time,
...
prompt
prompt Creating type WM_CONCAT_IMPL
prompt ============================
prompt
...
prompt
prompt Creating function FN_GETPY
prompt ==========================
prompt
...
prompt
prompt Creating function F_SPLITCLOB4
prompt ==============================
prompt
...
prompt
prompt Creating function IS_QUESTION_TEMPALTE
prompt ======================================
prompt
...
prompt
prompt Creating function WM_CONCAT
prompt ===========================
prompt
...
prompt
prompt Creating type body WM_CONCAT_IMPL
prompt =================================
prompt
...
CREATE OR REPLACE FUNCTION FIND_IN_SET(piv_str1 varchar2, piv_str2 varchar2, p_sep varchar2 := ' ',p_sep2 varchar2 := '  ')
RETURN varchar2 IS
/*
该函数用于判断 piv_str1 是否在 piv_str2 中，p_sep 截取内容为英文逗号，p_sep2 截取内容为中文逗号
如果是，作为条件，返回对应数据，如果piv_str1 不在piv_str2 或piv_str2 为空字符串，则返回值为 000999 。
*/
  l_idx    number:=0; -- 用于计算piv_str2中分隔符的位置
  str      varchar2(500);  -- 根据分隔符截取的子字符串
  piv_str  varchar2(500) := replace(piv_str2,' ',' '); -- 将piv_str2赋值给piv_str ，并去掉空格
  res  varchar2(500):='000999'; -- 返回结果
  loopIndex number:=0;
BEGIN
-- 如果piv_str中没有分割符，直接判断piv_str1和piv_str是否相等，相等 返回对应数据
IF instr(piv_str, p_sep, 1) = 0 and instr(piv_str, p_sep2, 1) = 0 THEN
...
-- 当piv_str中还有分隔符时
   ...
   -- 判断 str 和piv_str1 是否相等，相等 返回对应数据 并结束循环判断
      ...
      ELSE
   -- 当截取后的piv_str 中不存在分割符时，判断piv_str和piv_str1是否相等，相等 返回对应数据
        IF piv_str = piv_str1 THEN
           --res:= loopIndex;
           res:= piv_str1;
        END IF;
...
-- 当piv_str中还有分隔符时
   ...
   -- 判断 str 和piv_str1 是否相等，相等 返回对应数据 并结束循环判断
         IF str = piv_str1 THEN
           ...
           EXIT;
         END IF;
        piv_str := substr(piv_str,l_idx+length(p_sep2));
      ELSE
   -- 当截取后的piv_str 中不存在分割符时，判断piv_str和piv_str1是否相等，相等 返回对应数据
        IF piv_str = piv_str1 THEN
           --res:= loopIndex;
